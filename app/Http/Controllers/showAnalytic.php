...
class showAnalytic extends Controller
{
    ...
    public function show(){
        ...
        $array3=[($user->score->AIQ_P_)*100,($user->score->AIQ_S_)*100,($user->score->qa_)*100];
        ...
        $array5 = [($user->score->cck_)*100,($user->score->actual)*100,($user->score->cck_goal)*100];
        $response_cck_goal= json_encode($array5);
        $array7 = [($user->score->PP_)*100,];
        // Dpp //
        $response_dpp = json_encode($array7);
        //Minor Charts//
        $array6 = [($user->score->i90_)*100];
    ...
    return view('dashboard.analytic')->with('user',$user)
                                     ...
                                     ->with('response_dpp',$response_dpp)
